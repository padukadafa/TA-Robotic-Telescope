...
Sistem mekanik dibangun menggunakan teknologi cetak 3D dengan material PLA 2+ untuk menyeimbangkan antara berat dan kekuatan. Mounting \textit{Alt-Azimuth} didesain untuk menopang Tabung Teleskop Optik (OTA) reflektor Newtonian.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{figure/teleskop_reflektor.jpg}
    \caption{Implementasi Fisik Teleskop Robotik}
    \label{fig:4.hardware_realization}
\end{figure}

Sistem elektronik dipusatkan pada \textit{enclosure} yang memuat:
\begin{enumerate}[noitemsep]
    \item \textbf{Mikrokontroler ESP32}: Sebagai otak utama pemrosesan data dan komunikasi WiFi.
    \item \textbf{Driver Motor A4988}: Mengendalikan motor stepper NEMA 17 dengan mode \textit{microstepping} 1/16 untuk kehalusan gerakan.
    \item \textbf{Sensor Unit}: Modul MPU6050 (IMU), QMC5883L (Magnetometer), serta dua buah encoder magnetik AS5600 untuk pembacaan sudut \textit{Altitude} dan \textit{Azimuth}.
\end{enumerate}
